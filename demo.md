...
description: The Wickra indicator core that Wickra Synth is built on, running in your browser — powered by the wickra-wasm bundle from npm.
---

# The core, live

Every line below is computed by **`wickra-wasm`** — the indicator core
Wickra Synth is built on — running inside your browser via WebAssembly:
no server, no API call, no install. The same Rust kernel that computes every
indicator Wickra Synth reads is producing every value you see.
...
the [API pages here](/api/rust) are Wickra Synth's.
